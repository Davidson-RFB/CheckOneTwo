...
const User = {
  ...
  findByColumn: async (col, val, params) => {
    params = params || {};
    const query = ['SELECT id, email, name FROM users WHERE $1 = $2'];
    if (params.per_page && params.page) {
      query.push(`LIMIT ${params.per_page} OFFSET ${(params.page - 1) * params.per_page}`);
    }
    const result = await db.query(query.join(' '), [col, val]);
    return result.rows;
  },
  create: async (user) => {
  ...
  update: async (user) => {
    ...
    const result = await db.query('UPDATE users SET (email, name, updated_at) = ($2, $3, $4) WHERE id = $1 RETURNING *', [user.id, user.email, user.name, now]);
    return result.rows[0];
  },
};
